  ...
  else
  {
    ...
    switch (type)
    {
      ...
      case UTF_8_STRING_PAIR:
        len = 2 + prop->value.pair.key.len;
        len += 2 + prop->value.pair.value.len;
        break;
    }
  ...
  if (type >= BYTE && type <= UTF_8_STRING_PAIR)
  {
    ...
    switch (type)
    {
      ...
      case UTF_8_ENCODED_STRING:
        ...
        break;
      case UTF_8_STRING_PAIR:
        writeMQTTLenString(pptr, prop->value.pair.key);
        writeMQTTLenString(pptr, prop->value.pair.value);
        rc = prop->value.pair.key.len + prop->value.pair.value.len + 4; /* include length fields */
        break;
    }
  }
  ...
  if (type >= BYTE && type <= UTF_8_STRING_PAIR)
  {
    switch (type)
    {
      ...
      case UTF_8_ENCODED_STRING:
        ...
        break;
      case UTF_8_STRING_PAIR:
        len = MQTTLenStringRead(&prop->value.pair.key, pptr, enddata);
        len += MQTTLenStringRead(&prop->value.pair.value, pptr, enddata);
        break;
    }
  }
